...
var fileIOMethods = []*Builtin{
	...
	{Name: "path_join", Callback: pathJoin}, // Joins path elements
	{Name: "path_base", Callback: pathBase}, // Returns the last element of path
	{Name: "path_dir", Callback: pathDir},   // Returns all but the last element of path
	{Name: "path_ext", Callback: pathExt},   // Returns the file name extension
	{Name: "path_abs", Callback: pathAbs},   // Returns an absolute representation of path
	{Name: "glob", Callback: glob},          // Returns files matching a pattern
	{Name: "copy_file", Callback: copyFile}, // Copies a file from src to dst
}
...
// Syntax: path_join(elem1, elem2, ...)
// Example:
//
//	var fullPath = path_join("home", "user", "documents", "file.txt");
//
// This would set fullPath to "home/user/documents/file.txt" on Unix-like systems.
func pathJoin(rt Runtime, writer io.Writer, args ...GoMixObject) GoMixObject {
	if len(args) == 0 {
		return &String{Value: ""}
	}
	pathElements := make([]string, len(args))
	for i, arg := range args {
		pathElements[i] = arg.ToString()
	}
	return &String{Value: filepath.Join(pathElements...)}
}

// pathBase returns the last element of path.
// Syntax: path_base(path)
// Example:
//
//	var fileName = path_base("/home/user/documents/file.txt");
//
// This would set fileName to "file.txt".
func pathBase(rt Runtime, writer io.Writer, args ...GoMixObject) GoMixObject {
	if len(args) != 1 {
		return createError("ERROR: path_base expects 1 argument")
	}
	return &String{Value: filepath.Base(args[0].ToString())}
}

// pathDir returns all but the last element of path, typically the path's directory.
// Syntax: path_dir(path)
// Example:
//
//	var dirPath = path_dir("/home/user/documents/file.txt");
//
// This would set dirPath to "/home/user/documents".
func pathDir(rt Runtime, writer io.Writer, args ...GoMixObject) GoMixObject {
	if len(args) != 1 {
		return createError("ERROR: path_dir expects 1 argument")
	}
	return &String{Value: filepath.Dir(args[0].ToString())}
}

// pathExt returns the file name extension used by path.
// Syntax: path_ext(path)
// Example:
//
//	var ext = path_ext("/home/user/documents/file.txt");
//
// This would set ext to ".txt".
func pathExt(rt Runtime, writer io.Writer, args ...GoMixObject) GoMixObject {
	if len(args) != 1 {
		return createError("ERROR: path_ext expects 1 argument")
	}
	return &String{Value: filepath.Ext(args[0].ToString())}
}

// pathAbs returns an absolute representation of path.
// Syntax: path_abs(path)
// Example:
//
//	var absPath = path_abs("relative/path/to/file.txt");
//
// This would set absPath to the absolute path of "relative/path/to/file.txt" based on the current working directory.
func pathAbs(rt Runtime, writer io.Writer, args ...GoMixObject) GoMixObject {
	if len(args) != 1 {
		return createError("ERROR: path_abs expects 1 argument")
	}
	absPath, err := filepath.Abs(args[0].ToString())
	if err != nil {
		return createError("ERROR: could not determine absolute path: %v", err)
	}
	return &String{Value: absPath}
}
...
//
// This would set txtFiles to an array of all .txt files in the current directory.
// eg. ["notes.txt", "report.txt"]
func glob(rt Runtime, writer io.Writer, args ...GoMixObject) GoMixObject {
	if len(args) != 1 {
		return createError("ERROR: glob expects 1 argument (pattern)")
	}
	pattern := args[0].ToString()
	matches, err := filepath.Glob(pattern)
	if err != nil {
		return createError("ERROR: glob failed: %v", err)
	}
...
//
// Syntax: copy_file(src, dst)
func copyFile(rt Runtime, writer io.Writer, args ...GoMixObject) GoMixObject {
	if len(args) != 2 {
		return createError("ERROR: copy_file expects 2 arguments (src, dst)")
	}
	src := args[0].ToString()
	dst := args[1].ToString()
	...
	if err != nil {
		return createError("ERROR: could not open source file '%s': %v", src, err)
	}
	defer sourceFile.Close()

	destFile, err := os.Create(dst)
	if err != nil {
		return createError("ERROR: could not create destination file '%s': %v", dst, err)
	}
	defer destFile.Close()

	_, err = io.Copy(destFile, sourceFile)
	if err != nil {
		return createError("ERROR: could not copy file: %v", err)
	}
	return &Nil{}
}
